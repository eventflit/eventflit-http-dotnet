...
namespace EventflitServer.Tests.UnitTests
{
    ...
    public class When_Triggering_an_Event
    {
        ...
        public void Setup()
        {
            ...
            _subEventflitClient.ExecutePostAsync(Arg.Any<IEventflitRestRequest>()).Returns(Task.FromResult(new TriggerResult(_v8ProtocolSuccessfulResponse, TriggerResultHelper.TRIGGER_RESPONSE_JSON)));
        }

        [Test]
        public async void url_resource_is_in_expected_format()
        {
            await _eventflit.TriggerAsync(_channelName, _eventName, _eventData);

#pragma warning disable 4014
            _subEventflitClient.Received().ExecutePostAsync(
...
#pragma warning disable 4014
            _subEventflitClient.Received().ExecutePostAsync(Arg.Any<IEventflitRestRequest>());
#pragma warning restore 4014
        }
...
#pragma warning restore 4014
        ...
        {
            await _eventflit.TriggerAsync(new[] { "fish", "pie" }, _eventName, _eventData);
...
#pragma warning restore 4014
        ...
        public async void on_a_single_channel_the_socket_id_parameter_should_be_present_in_the_querystring()
        {
            ...
            await _eventflit.TriggerAsync(_channelName, _eventName, _eventData, new TriggerOptions()
                    {
...
#pragma warning disable 4014
            _subEventflitClient.Received().ExecutePostAsync(Arg.Is<IEventflitRestRequest>(
#pragma warning restore 4014
                x => CheckRequestContainsSocketIdParameter(x, expectedSocketId)
                )
            );
        ...
        public async void on_a_multiple_channels_the_socket_id_parameter_should_be_present_in_the_querystring()
        {
            ...
            await _eventflit.TriggerAsync(new[]{ "my-channel", "my-channel-2" }, _eventName, _eventData, new TriggerOptions()
                    {
                        SocketId = expectedSocketId
                    });
...
#pragma warning restore 4014
		...
		public async void channel_names_in_array_must_be_validated()
		{
		    ...
		    try
		    {
		        await _eventflit.TriggerAsync(new[] { "this_one_is_okay", "test_channel\n:" }, _eventName, _eventData);
		    }
		    catch (FormatException ex)
		    {
		        caughtException = ex;
        ...
        private bool CheckRequestContainsPayload(IEventflitRestRequest request, string channelName, string eventName, object eventData)
        {
        ...
        {
            var response = await _eventflit.TriggerAsync(_channelName, _eventName, _eventData, new TriggerOptions()
            {
                SocketId = socketId
            });
        ...
        {
            var response = await _eventflit.TriggerAsync(channelName, _eventName, _eventData);
        ...
        {
            var response = await _eventflit.TriggerAsync(events);

            return response;
        }

        private static bool CheckRequestContainsSocketIdParameter(IEventflitRestRequest request, string expectedSocketId)
        {
